...
{
    class TGCWebScrapingProdConsMethod
    {
        ...
        public void RunFlow(int nPages, bool chooseSN)
        {
            ...
            Task ProduceMthd = Task.Run(() => Produce(nPages));
            Task MountMthd = Task.Run(() => Mount());
            Task ConsumeMthd = Task.Run(() => Consume(chooseSN));

            Task.WaitAll(ProduceMthd, MountMthd, ConsumeMthd);
        }

        private void Produce(int nPages)
        {
        ...
        private List<string> ListPages(int nPages)
        {
